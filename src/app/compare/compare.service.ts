...
const SERVICE_URL:string = 'https://<aws>/<service>';
...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session)=>{
      ...
      this.http.post(SERVICE_URL, data, {
          headers: new Headers({
  ...
  onRetrieveData(all = true) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session)=>{
      ...
      this.http.get(SERVICE_URL + urlParam + queryParam, {
  ...
  onDeleteData() {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session)=>{
      ...
      let queryParam = '?accessToken=' + session.getAccessToken().getJwtToken();
      this.http.delete(SERVICE_URL + queryParam, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
      .subscribe(
         (data) => {
